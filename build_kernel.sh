...
KERNEL_PATCH_VER=${KERNEL_PATCH_VER:-"5.15.12"}
KERNEL_SUB_VER=${KERNEL_SUB_VER:-"051512"}
...
if [ ${KERNEL_TYPE} == "rt" ]; then
    ...
    if [ ${KERNEL_BASE_VER} == "5.16" ]; then
        cp -v ${CUSTOM_PATCH_PATH}/rt/${KERNEL_BASE_VER}/patch-5.16-rc6-rt12.patch .;
        patch -p1 < ./patch-5.16-rc6-rt12.patch;
    elif [ ${KERNEL_BASE_VER} == "5.15" ]; then
        cp -v ${CUSTOM_PATCH_PATH}/rt/${KERNEL_BASE_VER}/patch-5.15.10-rt24.patch .;
        patch -p1 < ./patch-5.15.10-rt24.patch;
        ...
        cp -v ${CUSTOM_PATCH_PATH}/rt/${KERNEL_BASE_VER}/patch-5.10.87-rt59.patch .;
        patch -p1 < ./patch-5.10.87-rt59.patch;
    elif [ ${KERNEL_BASE_VER} == "5.4" ]; then
        cp -v ${CUSTOM_PATCH_PATH}/rt/${KERNEL_BASE_VER}/patch-5.4.161-rt67.patch .;
        patch -p1 < ./patch-5.4.161-rt67.patch;
    fi
fi

if [ ${KERNEL_BASE_VER} == "5.16" ]; then   # Latest -rc, currently in development
    echo "*** Copying and applying arch patches.. ✓";
    cp -v ${LUCJAN_PATCH_PATH}/5.15/arch-patches-v11-sep/*.patch .;
    ...
    cp -v ${LUCJAN_PATCH_PATH}/5.15/cpufreq-patches-v6/*.patch .;
    ...
    cp -v ${LUCJAN_PATCH_PATH}/5.15/fixes-miscellaneous-v9-sep/*.patch .;
    ...
    patch -p1 < ./0005-mac80211-rate-replace-WARN_ON-with-pr_warn.patch;
    ...
    if [ ${KERNEL_TYPE} != "rt" ]; then
        ...
        cp -v ${LUCJAN_PATCH_PATH}/5.15/lrng-patches-v2/*.patch .;
        patch -p1 < ./0001-lrng-patches.patch;
    fi
    #### The patches below from 5.15 do not patch cleanly in 5.16-rc1 ####
    # echo "*** Copying and applying amd64 patches.. ✓";
    # cp -v ${LUCJAN_PATCH_PATH}/5.15/amd64-patches/*.patch .;
    # patch -p1 < ./0001-amd64-patches.patch;
    # echo "*** Copying and applying bbr2 patches.. ✓";
    # cp -v ${LUCJAN_PATCH_PATH}/5.15/bbr2-patches/*.patch .;
    # patch -p1 < ./0001-bbr2-5.15-introduce-BBRv2.patch;
    # echo "*** Copying and applying lru patches.. ✓";
    # cp -v ${LUCJAN_PATCH_PATH}/5.15/lru-patches-pf-v3/*.patch .;
    ...
    patch -p1 < ./0027-clearlinux-5.15-backport-patches-from-clearlinux-rep.patch;
    echo "*** Copying and applying cpufreq patches.. ✓";
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/cpufreq-patches-v6/*.patch .;
    patch -p1 < ./0001-cpufreq-patches.patch;
    echo "*** Copying an applying graysky patches.. ✓";
    ...
    if [ ${KERNEL_TYPE} == "rt" ]; then
        ...
        cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/damon-patches-v8/*.patch .;
        patch -p1 < ./0001-damon-patches.patch;
        echo "*** Copying and applying ksmbd patches.. ✓";
        cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/ksmbd-patches-v14/*.patch .;
...
# Examples:
# 5.15.12-051512+customidle-generic
# 5.15.12-051512+customfull-generic
# 5.15.12-051512+customrt-generic
# Note: A hyphen between label and type (e.g. customidle -> custom-idle) causes problems with some parsers
# Because the final version name becomes: 5.15.12-051512+custom-idle-generic, so just keep it combined
...
#
# To uninstall a kernel: $ sudo apt purge *5.15.12-051512+customidle-generic*
