...
KERNEL_PATCH_VER=${KERNEL_PATCH_VER:-"5.16.14"}
KERNEL_SUB_VER=${KERNEL_SUB_VER:-"051614"}
...
if [ ${KERNEL_TYPE} == "rt" ]; then
    ...
    if [ ${KERNEL_BASE_VER} == "5.17" ]; then
        cp -v ${CUSTOM_PATCH_PATH}/rt/${KERNEL_BASE_VER}/patch-5.17-rc6-rt11.patch .;
        patch -p1 < ./patch-5.17-rc6-rt11.patch;
...
if [ ${KERNEL_BASE_VER} == "5.17" ]; then   # Latest rc, in development
    ...
    cp -v ${LUCJAN_PATCH_PATH}/5.16/fixes-miscellaneous-v6-sep/*.patch .;
    ...
    patch -p1 < ./0025-mm-Increment-kswapd_waiters-for-throttled-direct-rec.patch;
    patch -p1 < ./0026-mm-Lower-the-non-hugetlbpage-pageblock-size-to-reduc.patch;
    patch -p1 < ./0027-mm-Don-t-hog-the-CPU-and-zone-lock-in-rmqueue_bulk.patch;
...
# Examples:
# 5.16.14-051614+customidle-generic
# 5.16.14-051614+customfull-generic
# 5.16.14-051614+customrt-generic
# Note: A hyphen between label and type (e.g. customidle -> custom-idle) causes problems with some parsers
# Because the final version name becomes: 5.16.14-051614+custom-idle-generic, so just keep it combined
...
#
# To uninstall a kernel: $ sudo apt purge *5.16.14-051614+customidle-generic*
