...
KERNEL_PATCH_VER=${KERNEL_PATCH_VER:-"5.16.10"}
KERNEL_SUB_VER=${KERNEL_SUB_VER:-"051610"}
...
if [ ${KERNEL_TYPE} == "rt" ]; then
    ...
    if [ ${KERNEL_BASE_VER} == "5.17" ]; then
        cp -v ${CUSTOM_PATCH_PATH}/rt/${KERNEL_BASE_VER}/patch-5.17-rc4-rt7.patch .;
        patch -p1 < ./patch-5.17-rc4-rt7.patch;
...
if [ ${KERNEL_BASE_VER} == "5.17" ]; then   # Latest rc, in development
    ...
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/hwmon-patches-v11/*.patch .;
...
# Examples:
# 5.16.10-051610+customidle-generic
# 5.16.10-051610+customfull-generic
# 5.16.10-051610+customrt-generic
# Note: A hyphen between label and type (e.g. customidle -> custom-idle) causes problems with some parsers
# Because the final version name becomes: 5.16.10-051610+custom-idle-generic, so just keep it combined
...
#
# To uninstall a kernel: $ sudo apt purge *5.16.10-051610+customidle-generic*
