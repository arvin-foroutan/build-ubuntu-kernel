...
KERNEL_PATCH_VER=${KERNEL_PATCH_VER:-"6.1.9"}
KERNEL_SUB_VER=${KERNEL_SUB_VER:-"060109"}
...
if [ ${KERNEL_BASE_VER} == "6.1" ]; then    # Latest mainline
    ...
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/amd-pstate-epp-guided-patches-v4-all/*.patch .;
    ...
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/fixes-miscellaneous-v37/*.patch .;
    ...
    if [ ${KERNEL_TYPE} != "rt" ]; then
        ...
        cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/bcachefs-patches-v22/*.patch .;
...
# Examples:
# 6.1.9-060109+customidle-generic
# 6.1.9-060109+customfull-generic
# 6.1.9-060109+customrt-generic
# Note: A hyphen between label and type (e.g. customidle -> custom-idle) causes problems with some parsers
# Because the final version name becomes: 6.1.9-060109+custom-idle-generic, so just keep it combined
...
# To uninstall a kernel: sudo apt purge *6.1.9-060109+customidle-generic*
