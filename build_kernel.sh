...
KERNEL_PATCH_VER=${KERNEL_PATCH_VER:-"5.15.2"}
KERNEL_SUB_VER=${KERNEL_SUB_VER:-"051502"}
...
if [ ${KERNEL_TYPE} == "rt" ]; then
    ...
    if [ ${KERNEL_BASE_VER} == "5.16" ]; then
        cp -v ${CUSTOM_PATCH_PATH}/rt/${KERNEL_BASE_VER}/patch-5.16-rc1-rt2.patch .;
        patch -p1 < ./patch-5.16-rc1-rt2.patch;
    elif [ ${KERNEL_BASE_VER} == "5.15" ]; then
        cp -v ${CUSTOM_PATCH_PATH}/rt/${KERNEL_BASE_VER}/patch-5.15.3-rt21.patch .;
        patch -p1 < ./patch-5.15.3-rt21.patch;
        ...
        cp -v ${CUSTOM_PATCH_PATH}/rt/${KERNEL_BASE_VER}/patch-5.4.154-rt65.patch .;
        patch -p1 < ./patch-5.4.154-rt65.patch;
    fi
fi

if [ ${KERNEL_BASE_VER} == "5.16" ]; then   # Latest -rc, currently in development
    echo "*** Copying and applying arch patches.. ✓";
    cp -v ${LUCJAN_PATCH_PATH}/5.15/arch-patches-v2-sep/*.patch .;
    ...
    cp -v ${LUCJAN_PATCH_PATH}/5.15/fixes-miscellaneous-v4-sep/*.patch .;
...
# Examples:
# 5.15.2-051502+customidle-generic
# 5.15.2-051502+customfull-generic
# 5.15.2-051502+customrt-generic
# Note: A hyphen between label and type (e.g. customidle -> custom-idle) causes problems with some parsers
# Because the final version name becomes: 5.15.2-051502+custom-idle-generic, so just keep it combined
...
#
# To uninstall a kernel: $ sudo apt purge *5.15.2-051502+customidle-generic*
