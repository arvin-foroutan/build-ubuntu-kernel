...
KERNEL_PATCH_VER=${KERNEL_PATCH_VER:-"6.1.4"}
KERNEL_SUB_VER=${KERNEL_SUB_VER:-"060104"}
...
if [ ${KERNEL_BASE_VER} == "6.1" ]; then    # Latest mainline
    ...
    patch -p1 < ./0002-drm-sched-add-DRM_SCHED_FENCE_DONT_PIPELINE-flag.patch;
    patch -p1 < ./0003-drm-amdgpu-use-DRM_SCHED_FENCE_DONT_PIPELINE-for-VM-.patch;
    echo "*** Copying and applying aufs patches.. ✓";
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/aufs-patches/*.patch .;
    patch -p1 < ./0001-aufs-6.0-merge-v20221031.patch;
    echo "*** Copying and applying bbr2 patches.. ✓";
    ...
    patch -p1 < ./0001-mm-Change-dirty-writeback-defaults.patch;
    patch -p1 < ./0002-ZEN-mm-Lower-the-non-hugetlbpage-pageblock-size-to-r.patch;
    patch -p1 < ./0003-objtool-Remove-ANNOTATE_NOENDBR-on-ENDBR-warning.patch;
    patch -p1 < ./0004-sched-all-Change-all-BUG_ON-instances-in-the-schedul.patch;
    patch -p1 < ./0005-x86-retpoline-Be-sure-to-emit-INT3-after-JMP-reg.patch;
    patch -p1 < ./0006-zram-don-t-retry-compress-incompressible-page.patch;
    patch -p1 < ./0007-leds-trigger-Add-block-device-LED-trigger.patch;
    patch -p1 < ./0008-docs-Add-block-device-blkdev-LED-trigger-documentati.patch;
    patch -p1 < ./0009-sbitmap-fix-possible-io-hung-due-to-lost-wakeup.patch;
    patch -p1 < ./0010-sbitmap-remove-unnecessary-code-in-__sbitmap_queue_g.patch;
    patch -p1 < ./0011-sbitmap-Avoid-leaving-waitqueue-in-invalid-state-in-.patch;
    patch -p1 < ./0012-sbitmap-Use-atomic_long_try_cmpxchg-in-__sbitmap_que.patch;
    patch -p1 < ./0013-sbitmap-fix-batched-wait_cnt-accounting.patch;
    patch -p1 < ./0014-sbitmap-fix-lockup-while-swapping.patch;
    patch -p1 < ./0015-memcg-calling-reclaim_high-GFP_KERNEL-in-GFP_NOFS-co.patch;
    patch -p1 < ./0016-elevator-remove-redundant-code-in-elv_unregister_que.patch;
    patch -p1 < ./0017-blk-wbt-remove-unnecessary-check-in-wbt_enable_defau.patch;
    patch -p1 < ./0018-blk-wbt-make-enable_state-more-accurate.patch;
    patch -p1 < ./0019-blk-wbt-don-t-show-valid-wbt_lat_usec-in-sysfs-while.patch;
    patch -p1 < ./0020-elevator-add-new-field-flags-in-struct-elevator_queu.patch;
    patch -p1 < ./0021-blk-wbt-don-t-enable-throttling-if-default-elevator-.patch;
    patch -p1 < ./0022-Revert-blk-wbt-don-t-enable-throttling-if-default-el.patch;
    patch -p1 < ./0023-Revert-elevator-add-new-field-flags-in-struct-elevat.patch;
    patch -p1 < ./0024-Revert-blk-wbt-don-t-show-valid-wbt_lat_usec-in-sysf.patch;
    patch -p1 < ./0025-Revert-blk-wbt-make-enable_state-more-accurate.patch;
    patch -p1 < ./0026-Revert-blk-wbt-remove-unnecessary-check-in-wbt_enabl.patch;
    patch -p1 < ./0027-Revert-elevator-remove-redundant-code-in-elv_unregis.patch;
    patch -p1 < ./0028-elevator-remove-redundant-code-in-elv_unregister_que.patch;
    patch -p1 < ./0029-blk-wbt-remove-unnecessary-check-in-wbt_enable_defau.patch;
    patch -p1 < ./0030-blk-wbt-make-enable_state-more-accurate.patch;
    patch -p1 < ./0031-blk-wbt-don-t-show-valid-wbt_lat_usec-in-sysfs-while.patch;
    patch -p1 < ./0032-elevator-add-new-field-flags-in-struct-elevator_queu.patch;
    patch -p1 < ./0033-blk-wbt-don-t-enable-throttling-if-default-elevator-.patch;
    patch -p1 < ./0036-epoll-ep_autoremove_wake_function-should-use-list_de.patch;
    echo "*** Copying and applying spadfs patches.. ✓";
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/spadfs-patches-v2/*.patch .;
    patch -p1 < ./0001-spadfs-6.0-merge-v1.0.17.patch;
    echo "*** Copying and applying lucjan's zen patches.. ✓";
    ...
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/bitmap-patches-v2/*.patch .;
    patch -p1 < ./0001-bitmap-5.19-merge-changes-from-dev-tree.patch;
    echo "*** Copying and applying boot patches.. ✓";
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/boot-patches/*.patch .;
    patch -p1 < ./0001-boot-patches.patch;
    echo "*** Copying and applying cpu patches.. ✓";
    ...
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/ntfs3-patches-v4/*.patch .;
    ...
    patch -p1 < ./0001-rtw-5.19-add-rtw88-support.patch;
    ...
    echo "*** Copying and applying clearlinux patches.. ✓";
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/clearlinux-patches-v5-sep/*.patch .;
    ...
    patch -p1 < ./0013-add-boot-option-to-allow-unsigned-modules.patch;
    patch -p1 < ./0014-enable-stateless-firmware-loading.patch;
    patch -p1 < ./0015-migrate-some-systemd-defaults-to-the-kernel-defaults.patch;
    patch -p1 < ./0016-xattr-allow-setting-user.-attributes-on-symlinks-by-.patch;
    patch -p1 < ./0017-use-lfence-instead-of-rep-and-nop.patch;
    patch -p1 < ./0018-do-accept-in-LIFO-order-for-cache-efficiency.patch;
    patch -p1 < ./0019-port-locking-rwsem-spin-faster.patch;
    patch -p1 < ./0020-ata-libahci-ignore-staggered-spin-up.patch;
    patch -p1 < ./0021-print-CPU-that-faults.patch;
    patch -p1 < ./0022-fix-bug-in-ucode-force-reload-revision-check.patch;
    patch -p1 < ./0023-nvme-workaround.patch;
    patch -p1 < ./0024-don-t-report-an-error-if-PowerClamp-run-on-other-CPU.patch;
    patch -p1 < ./0025-Port-microcode-patches.patch;
    patch -p1 < ./0026-clearlinux-5.18-backport-patches-from-clearlinux-rep.patch;
    echo "*** Copying an applying cpu graysky patches.. ✓";
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/cpu-patches-sep/*.patch .;
    patch -p1 < ./0001-cpu-5.18-merge-graysky-s-patchset.patch;
    ...
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/pf-fixes/*.patch .;
    patch -p1 < ./0001-pf-patches.patch;
    echo "*** Copying and applying rtw88 patches.. ✓";
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/rtw88-patches/*.patch .;
    patch -p1 < ./0001-rtw88-patches.patch;
    echo "*** Copying and applying spadfs patches.. ✓";
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/spadfs-patches-v3/*.patch .;
    patch -p1 < ./0001-spadfs-5.18-merge-v1.0.15.patch;
    echo "*** Copying and applying v4l2loopback patches.. ✓";
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/v4l2loopback-patches/*.patch .;
    patch -p1 < ./0001-media-v4l2-core-add-v4l2loopback.patch;
    echo "*** Copying and applying winesync patches.. ✓";
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/winesync-patches/*.patch .;
    patch -p1 < ./0001-winesync-patches.patch;
    ...
    echo "*** Copying and applying zram patches.. ✓";
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/zram-patches/*.patch .;
    patch -p1 < ./0001-zram-5.18-introduce-ZRAM-entropy-calculation.patch;
    echo "*** Copying and applying lucjan's zen patches.. ✓";
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/zen-patches/*.patch .;
    patch -p1 < ./0001-zen-patches.patch;
    ...
    if [ ${KERNEL_TYPE} != "rt" ]; then
        ...
        patch -p1 < ./0003-wait-Add-EXPORT_SYMBOL-for-__wake_up_pollfree.patch;
        patch -p1 < ./0004-mac80211-ignore-AP-power-level-when-tx-power-type-is.patch;
    ...
    echo "*** Copying and applying arch patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/arch-patches-v18/*.patch .;
    patch -p1 < ./0001-arch-patches.patch;
    echo "*** Copying and applying bbr2 patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/bbr2-patches-v3/*.patch .;
    patch -p1 < ./0001-bbr2-5.17-introduce-BBRv2.patch;
    echo "*** Copying and applying blk patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/blk-patches-v3/*.patch .;
    patch -p1 < ./0001-blk-patches.patch;
    echo "*** Copying and applying clearlinux patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/clearlinux-patches-v2/*.patch .;
    patch -p1 < ./0001-clearlinux-patches.patch;
    echo "*** Copying and applying clearlinux amd patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/clearlinux-amd-fixes/*.patch .;
    patch -p1 < ./0001-fix-amd-fails.patch;
    echo "*** Copying an applying cpu graysky patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/cpu-patches-sep/*.patch .;
    patch -p1 < ./0001-cpu-${KERNEL_BASE_VER}-merge-graysky-s-patchset.patch;
    patch -p1 < ./0002-init-Kconfig-enable-O3-for-all-arches.patch;
    patch -p1 < ./0003-init-Kconfig-add-O1-flag.patch;
    patch -p1 < ./0004-Makefile-Turn-off-loop-vectorization-for-GCC-O3-opti.patch;
    echo "*** Copying and applying fixes misc patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/fixes-miscellaneous-v12-sep/*.patch .;
    patch -p1 < ./0001-net-sched-allow-configuring-cake-qdisc-as-default.patch;
    patch -p1 < ./0002-infiniband-Fix-__read_overflow2-error-with-O3-inlini.patch;
    patch -p1 < ./0003-pci-Enable-overrides-for-missing-ACS-capabilities.patch;
    patch -p1 < ./0004-scsi-sd-Optimal-I-O-size-should-be-a-multiple-of-rep.patch;
    patch -p1 < ./0006-i2c-busses-Add-SMBus-capability-to-work-with-OpenRGB.patch;
    patch -p1 < ./0007-fm-5.17-port-mm-kswapd-patches.patch;
    patch -p1 < ./0008-Disable-stack-conservation-for-GCC.patch;
    patch -p1 < ./0009-openrgb-Deduplicate-piix4-setup-for-HUDSON2-KERNCZ-S.patch;
    patch -p1 < ./0010-kernel-cpu.c-fix-init_cpu_online.patch;
    ...
    echo "*** Copying and applying hwmon patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/hwmon-patches-v8/*.patch .;
    patch -p1 < ./0001-hwmon-5.17-patches.patch;
    echo "*** Copying and applying lqx patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/lqx-patches/*.patch .;
    patch -p1 < ./0001-lqx-patches.patch;
    echo "*** Copying and applying spadfs patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/spadfs-patches/*.patch .;
    ...
    patch -p1 < ./0001-v4l2loopback-patches.patch;
    echo "*** Copying and applying lucjan's xanmod patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/xanmod-patches-v4/*.patch .;
    patch -p1 < ./0001-xanmod-miscellaneous.patch;
    echo "*** Copying and applying lucjan's zen patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/zen-patches-v2/*.patch .;
    patch -p1 < ./0001-zen-patches.patch;
    echo "*** Copying and applying misc xanmod tweaks.. ✓";
    cp -v ${XANMOD_PATCH_PATH}/linux-5.18.y-xanmod/xanmod/*.patch .;
    patch -p1 < ./0009-XANMOD-mm-vmscan-vm_swappiness-30-decreases-the-amou.patch;
    ...
    if [ ${KERNEL_TYPE} != "rt" ]; then
        ...
        cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/btrfs-patches-v7/*.patch .;
        patch -p1 < ./0001-btrfs-patches.patch;
        echo "*** Copying and applying lru patches.. ✓";
        cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/lru-patches-pf-v3/*.patch .;
        patch -p1 < ./0001-lru-patches.patch;
        echo "*** Copying and applying pf patches.. ✓";
        cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/pf-fixes-v7/*.patch .;
    ...
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/amd64-patches/*.patch .;
    patch -p1 < ./0001-amd64-patches.patch;
    echo "*** Copying and applying arch patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/arch-patches-v5/*.patch .;
    patch -p1 < ./0001-arch-patches.patch;
    echo "*** Copying and applying bbr2 patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/bbr2-patches-v2/*.patch .;
    patch -p1 < ./0001-bbr2-patches.patch;
    echo "*** Copying and applying blk patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/blk-patches-v4/*.patch .;
    patch -p1 < ./0001-blk-patches.patch;
    echo "*** Copying and applying block patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/block-patches/*.patch .;
    patch -p1 < ./0001-block-patches.patch;
    echo "*** Copying and applying btrfs patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/btrfs-patches-v10/*.patch .;
    patch -p1 < ./0001-btrfs-patches.patch;
    echo "*** Copying and applying clearlinux patches.. ✓";
    ...
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/cpufreq-patches-v5/*.patch .;
    patch -p1 < ./0001-cpufreq-patches.patch;
    echo "*** Copying and applying f2fs patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/f2fs-patches-v4/*.patch .;
    patch -p1 < ./0001-f2fs-patches.patch;
    echo "*** Copying and applying fixes misc patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/fixes-miscellaneous-v7-sep/*.patch .;
    patch -p1 < ./0001-net-sched-allow-configuring-cake-qdisc-as-default.patch;
    patch -p1 < ./0002-infiniband-Fix-__read_overflow2-error-with-O3-inlini.patch;
    patch -p1 < ./0003-pci-Enable-overrides-for-missing-ACS-capabilities.patch;
    patch -p1 < ./0004-scsi-sd-Optimal-I-O-size-should-be-a-multiple-of-rep.patch;
    ...
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/hwmon-patches-v12/*.patch .;
    patch -p1 < ./0001-hwmon-patches.patch;
    echo "*** Copying and applying lqx patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/lqx-patches/*.patch .;
    patch -p1 < ./0001-lqx-patches.patch;
    echo "*** Copying and applying lrng patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/lrng-patches/*.patch .;
    patch -p1 < ./0001-lrng-patches.patch;
    echo "*** Copying and applying net patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/net-patches-v3/*.patch .;
    patch -p1 < ./0001-net-patches.patch;
    echo "*** Copying and applying smbus patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/smbus-patches-v2/*.patch .;
    ...
    patch -p1 < ./0001-v4l2loopback-patches.patch;
    echo "*** Copying and applying lucjan's xanmod patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/xanmod-patches-v2/*.patch .;
    patch -p1 < ./0001-xanmod-patches.patch;
    echo "*** Copying and applying lucjan's zen patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/zen-patches/*.patch .;
    patch -p1 < ./0001-zen-patches.patch;
    echo "*** Copying and applying zstd patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/zstd-patches-v4/*.patch .;
    patch -p1 < ./0001-zstd-patches.patch;
    echo "*** Copying and applying disable memory compaction patch.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/tweaks/5.13-disable-compaction-on-unevictable-pages.patch .;
    patch -p1 < ./5.13-disable-compaction-on-unevictable-pages.patch;
    ...
    if [ ${KERNEL_TYPE} != "rt" ]; then
        echo "*** Copying and applying lru patches.. ✓";
        cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/lru-patches-pf/*.patch .;
        patch -p1 < ./0001-lru-patches.patch;
        echo "*** Copying and applying pf patches.. ✓";
        cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/pf-patches-v3/*.patch .;
        patch -p1 < ./0001-pf-patches.patch;
    fi
    ...
    if [ ${KERNEL_TYPE} != "rt" ]; then
        ...
        cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/damon-patches-v10/*.patch .;
        patch -p1 < ./0001-damon-patches.patch;
        echo "*** Copying and applying lrng patches.. ✓";
        cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/lrng-patches-v3/*.patch .;
        patch -p1 < ./0001-lrng-patches.patch;
        echo "*** Copying and applying lru patches.. ✓";
        cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/lru-patches-pf-v4/*.patch .;
        patch -p1 < ./0001-lru-patches.patch;
    fi
    ...
    echo "*** Copying and applying aufs patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/aufs-patches/*.patch .;
    patch -p1 < ./0001-aufs-20211018.patch;
    echo "*** Copying and applying bbr2 patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/bbr2-patches/*.patch .;
    patch -p1 < ./0001-bbr2-5.14-introduce-BBRv2.patch;
    echo "*** Copying and applying bcachefs patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/bcachefs-patches/*.patch .;
    patch -p1 < ./0001-bcachefs-5.14-introduce-bcachefs-patchset.patch;
    echo "*** Copying and applying bfq patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/bfq-patches-v2-sep/*.patch .;
    ...
    echo "*** Copying and applying btrfs patches.. ✓";
    if [ ${KERNEL_TYPE} == "rt" ]; then
        cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/btrfs-patches-v2/*.patch .;
    else
        cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/btrfs-patches-v10/*.patch .;
    fi
    patch -p1 < ./0001-btrfs-patches.patch;
    echo "*** Copying and applying clearlinux patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/clearlinux-patches/*.patch .;
    patch -p1 < ./0001-clearlinux-patches.patch;
    echo "*** Copying and applying cpufreq patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/cpufreq-patches-v2/*.patch .;
    patch -p1 < ./0001-cpufreq-patches.patch;
    echo "*** Copying and applying cpu graysky patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/cpu-patches/*.patch .;
    patch -p1 < ./0001-cpu-patches.patch;
    echo "*** Copying and applying fixes misc patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/fixes-miscellaneous-v8-sep/*.patch .;
    patch -p1 < ./0001-net-sched-allow-configuring-cake-qdisc-as-default.patch;
    patch -p1 < ./0002-infiniband-Fix-__read_overflow2-error-with-O3-inlini.patch;
    ...
    echo "*** Copying and applying hwmon patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/hwmon-patches-v7/*.patch .;
    patch -p1 < ./0001-hwmon-patches.patch;
    echo "*** Copying and applying ksmbd patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/ksmbd-patches-v21/*.patch .;
    patch -p1 < ./0001-ksmbd-patches.patch;
    echo "*** Copying and applying lqx patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/lqx-patches-sep/*.patch .;
    patch -p1 < ./0001-zen-Allow-MSR-writes-by-default.patch;
    patch -p1 < ./0002-PCI-Add-Intel-remapped-NVMe-device-support.patch;
    echo "*** Copying and applying lrng patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/lrng-patches-v3/*.patch .;
    patch -p1 < ./0001-lrng-patches.patch;
    echo "*** Copying and applying ntfs3 patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/ntfs3-patches-v14/*.patch .;
    patch -p1 < ./0001-ntfs3-patches.patch;
    echo "*** Copying and applying pf patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/pf-patches-v8-sep/*.patch .;
    ...
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/security-patches-sep/*.patch .;
    ...
    echo "*** Copying and applying spectre patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/spectre-patches-sep/*.patch .;
    ...
    echo "*** Copying and applying v4l2loopback patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/v4l2loopback-patches/*.patch .;
    patch -p1 < ./0001-v4l2loopback-5.14-merge-v0.12.5.patch;
    echo "*** Copying and applying writeback patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/writeback-patches/*.patch .;
    patch -p1 < ./0001-writeback-patches.patch;
    echo "*** Copying and applying xanmod patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/xanmod-patches-v2-sep/*.patch .;
    patch -p1 < ./0001-sched-autogroup-Add-kernel-parameter-and-config-opti.patch;
    patch -p1 < ./0002-netfilter-Add-full-cone-NAT-support.patch;
    echo "*** Copying and applying zen patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/zen-patches-v2-sep/*.patch .;
    patch -p1 < ./0001-ZEN-Add-VHBA-driver.patch;
    echo "*** Copying and applying zstd patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/zstd-patches-v2/*.patch .;
    patch -p1 < ./0001-zstd-patches.patch;
    echo "*** Copying and applying zstd upstream patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/zstd-upstream-patches-v7/*.patch .;
    patch -p1 < ./0001-zstd-upstream-patches.patch;
    # Misc / Tweaks
    sed -i 's/sched_nr_migrate = 32/sched_nr_migrate = 256/g' ./kernel/sched/core.c;
    echo "*** Copying and applying cjktty 5.13 patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/5.13/cjktty-patches/*.patch .;
    ...
    if [ ${KERNEL_TYPE} != "rt" ]; then
        ...
        cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/futex-zen-patches-v2/*.patch .;
        patch -p1 < ./0001-futex-resync-from-gitlab.collabora.com.patch;
        echo "*** Copying and applying lru patches.. ✓";
        cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/lru-zen-patches-v4/*.patch .;
        patch -p1 < ./0001-lru-zen-patches.patch;
        echo "*** Copying and applying ksm patches.. ✓";
        cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/ksm-patches-sep/*.patch .;
    ...
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/alsa-patches-v2/*.patch .;
    patch -p1 < ./0001-alsa-patches.patch;
    echo "*** Copying and applying arch patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/arch-patches-v6/*.patch .;
    patch -p1 < ./0001-ZEN-Add-sysctl-and-CONFIG-to-disallow-unprivileged-C.patch;
    echo "*** Copying and applying bbr2 patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/bbr2-patches-v2/*.patch .;
    patch -p1 < ./0001-bbr2-patches.patch;
    echo "*** Copying and applying bfq patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/bfq-patches-v7-sep/*.patch .;
    ...
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/cjktty-patches/*.patch .;
    patch -p1 < ./0001-cjktty-5.13-initial-import-from-https-github.com-zhm.patch;
    echo "*** Copying and applying clearlinux patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/clearlinux-patches-v2-sep/*.patch .;
    patch -p1 < ./0001-i8042-decrease-debug-message-level-to-info.patch;
    ...
    echo "*** Copying and applying ksmbd patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/ksmbd-patches-v3/*.patch .;
    patch -p1 < ./0001-ksmbd-patches.patch;
    echo "*** Copying and applying lrng patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/lrng-patches/*.patch .;
    patch -p1 < ./0001-lrng-patches.patch;
    echo "*** Copying and applying lru-mm patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/lru-patches-v8/*.patch .;
    patch -p1 < ./0001-lru-patches.patch;
    echo "*** Copying and applying ntfs3 patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/ntfs3-patches-v3/*.patch .;
    patch -p1 < ./0001-ntfs3-patches.patch;
    echo "*** Copying and applying pf patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/pf-patches-v12-sep/*.patch .;
    ...
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/swap-patches/*.patch .;
    ...
    patch -p1 < ./0001-writeback-patches.patch;
    echo "*** Copying and applying xanmod patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/xanmod-patches-v3/*.patch .;
    patch -p1 < ./0001-xanmod-patches.patch;
    ...
    echo "*** Copying and applying zstd patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/zstd-patches-v5/*.patch .;
    patch -p1 < ./0001-zstd-patches.patch;
    echo "*** Copying and applying zen patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/zen-patches-sep/*.patch .;
    patch -p1 < ./0001-ZEN-Add-VHBA-driver.patch;
    ...
    patch -p1 < ./0004-mm-set-8-megabytes-for-address_space-level-file-read.patch;
    echo "*** Copying and applying lqx patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/lqx-patches-v2-sep/*.patch .;
    patch -p1 < ./0001-zen-Allow-MSR-writes-by-default.patch;
    patch -p1 < ./0002-PCI-Add-Intel-remapped-NVMe-device-support.patch;
    echo "*** Copying and applying cfs xanmod tweaks patch.. ✓";
    ...
    if [ ${KERNEL_TYPE} == "rt" ]; then
        ...
        cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/futex2-zen-patches-v2/*.patch .;
        ...
        echo "*** Copying and applying ksm patches.. ✓";
        cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/ksm-patches/*.patch .;
    ...
    patch -p1 < ./0001-block-patches.patch;
    echo "*** Copying and applying btrfs patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/btrfs-patches-v14-sep/*.patch .;
    ...
    patch -p1 < ./0001-lqx-patches.patch;
    echo "*** Copying and applying mm patches.. ✓";
    cp -v ${CUSTOM_PATCH_PATH}/lucjan/${KERNEL_BASE_VER}/mm-patches-v4/*.patch .;
