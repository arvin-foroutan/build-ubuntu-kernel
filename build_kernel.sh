...
KERNEL_BASE_VER=${KERNEL_BASE_VER:-"6.18"}
KERNEL_PATCH_VER=${KERNEL_PATCH_VER:-"6.18.5"}
KERNEL_SUB_VER=${KERNEL_SUB_VER:-"061805"}
...
if [ ${UBUNTU_PATCHES} == "yes" ]; then
    ...
    elif [ ${KERNEL_BASE_VER} == "6.17" ] ||
         [ ${KERNEL_BASE_VER} == "6.18" ]; then
        KERNEL_BASE_VER_OVERRIDE=6.17+;
    else
...
if [ ${KERNEL_BASE_VER} == "6.18" ]; then   # Latest mainline
    ...
    patch -p1 < ./0005-rust_binder-correctly-handle-FDA-objects-of-length-z.patch;
    ...
    patch -p1 < ./0019-ALSA-hda-tas2781-Skip-UEFI-calibration-on-ASUS-ROG-X.patch;
    patch -p1 < ./0020-sched-fair-Drop-useless-cpumask_empty-in-find_energy.patch;
    patch -p1 < ./0021-sched-fair-Simplify-task_numa_find_cpu.patch;
    patch -p1 < ./0022-drm-amd-display-Bump-the-HDMI-clock-to-340MHz.patch;
...
elif [ ${KERNEL_BASE_VER} == "6.17" ]; then # Latest stable
    echo "*** Copying and applying adios io patches.. ✓";
    ...
    echo "*** Copying and applying asus patches.. ✓";
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/asus-patches-v9/*.patch .;
    patch -p1 < ./0001-asus-patches.patch;
    echo "*** Copying and applying block patches.. ✓";
