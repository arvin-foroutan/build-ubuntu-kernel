...
KERNEL_PATCH_VER=${KERNEL_PATCH_VER:-"5.4.146"}
KERNEL_SUB_VER=${KERNEL_SUB_VER:-"0504146"}
...
if [ ${KERNEL_BASE_VER} == "5.15" ]; then
    ...
    cp -v ${LUCJAN_PATCH_PATH}/5.14/zstd-upstream-patches-v3/*.patch .;
    ...
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/ntfs3-patches-v6/*.patch .;
    ...
    patch -p1 < ./0001-zstd-patches.patch;
    echo "*** Copying and applying zstd upstream patches.. ✓";
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/zstd-upstream-patches-v3/*.patch .;
    patch -p1 < ./0001-zstd-upstream-patches.patch;
    # Misc / Tweaks
    sed -i 's/sched_nr_migrate = 32/sched_nr_migrate = 256/g' ./kernel/sched/core.c;
    echo "*** Copying and applying cjktty 5.13 patches.. ✓";
...
# Examples:
# 5.4.146-0504146+customidle-generic
# 5.4.146-0504146+customfull-generic
# 5.4.146-0504146+customrt-generic
# Note: A hyphen between label and type (e.g. customidle -> custom-idle) causes problems with some parsers
# Because the final version name becomes: 5.4.146-0504146+custom-idle-generic, so just keep it combined
...
#
# To uninstall a kernel: $ sudo apt purge *5.4.146-0504146+customidle-generic*
