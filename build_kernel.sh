...
KERNEL_PATCH_VER=${KERNEL_PATCH_VER:-"5.4.155"}
KERNEL_SUB_VER=${KERNEL_SUB_VER:-"0504155"}
...
if [ ${KERNEL_TYPE} == "rt" ]; then
    ...
    if [ ${KERNEL_BASE_VER} == "5.15" ]; then
        cp -v ${CUSTOM_PATCH_PATH}/rt/${KERNEL_BASE_VER}/patch-5.15-rc6-rt12.patch .;
        patch -p1 < ./patch-5.15-rc6-rt12.patch;
...
if [ ${KERNEL_BASE_VER} == "5.15" ]; then   # Latest mainline, in -rc right now
    ...
    echo "*** Copying and applying cpufreq patches.. ✓";
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/cpufreq-patches-sep/*.patch .;
    patch -p1 < ./0001-x86-cpufreatures-add-AMD-Collaborative-Processor-Per.patch;
    patch -p1 < ./0002-x86-msr-add-AMD-CPPC-MSR-definitions.patch;
    patch -p1 < ./0003-ACPI-CPPC-Check-online-CPUs-for-determining-_CPC-is-.patch;
    patch -p1 < ./0004-ACPI-CPPC-add-cppc-enable-register-function.patch;
    ...
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/ksmbd-patches-v19/*.patch .;
...
# Examples:
# 5.4.155-0504155+customidle-generic
# 5.4.155-0504155+customfull-generic
# 5.4.155-0504155+customrt-generic
# Note: A hyphen between label and type (e.g. customidle -> custom-idle) causes problems with some parsers
# Because the final version name becomes: 5.4.155-0504155+custom-idle-generic, so just keep it combined
...
#
# To uninstall a kernel: $ sudo apt purge *5.4.155-0504155+customidle-generic*
