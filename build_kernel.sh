...
KERNEL_PATCH_VER=${KERNEL_PATCH_VER:-"5.4.149"}
KERNEL_SUB_VER=${KERNEL_SUB_VER:-"0504149"}
...
if [ ${KERNEL_TYPE} == "rt" ]; then
    ...
    if [ ${KERNEL_BASE_VER} == "5.15" ]; then
        cp -v ${CUSTOM_PATCH_PATH}/rt/${KERNEL_BASE_VER}/patch-5.15-rc2-rt4.patch .;
        patch -p1 < ./patch-5.15-rc2-rt4.patch;
...
if [ ${KERNEL_BASE_VER} == "5.15" ]; then   # Latest mainline, in -rc right now
    ...
    cp -v ${LUCJAN_PATCH_PATH}/${KERNEL_BASE_VER}/ntfs3-patches-v11/*.patch .;
...
# Examples:
# 5.4.149-0504149+customidle-generic
# 5.4.149-0504149+customfull-generic
# 5.4.149-0504149+customrt-generic
# Note: A hyphen between label and type (e.g. customidle -> custom-idle) causes problems with some parsers
# Because the final version name becomes: 5.4.149-0504149+custom-idle-generic, so just keep it combined
...
#
# To uninstall a kernel: $ sudo apt purge *5.4.149-0504149+customidle-generic*
