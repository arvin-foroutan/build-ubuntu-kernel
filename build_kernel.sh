...
KERNEL_PATCH_VER=${KERNEL_PATCH_VER:-"5.16.5"}
KERNEL_SUB_VER=${KERNEL_SUB_VER:-"051605"}
...
if [ ${KERNEL_TYPE} == "rt" ]; then
    ...
    if [ ${KERNEL_BASE_VER} == "5.17" ]; then
        cp -v ${CUSTOM_PATCH_PATH}/rt/${KERNEL_BASE_VER}/patch-5.17-rc2-rt3.patch .;
        patch -p1 < ./patch-5.17-rc2-rt3.patch;
        ...
        cp -v ${CUSTOM_PATCH_PATH}/rt/${KERNEL_BASE_VER}/patch-5.15.18-rt28.patch .;
        patch -p1 < ./patch-5.15.18-rt28.patch;
...
# Examples:
# 5.16.5-051605+customidle-generic
# 5.16.5-051605+customfull-generic
# 5.16.5-051605+customrt-generic
# Note: A hyphen between label and type (e.g. customidle -> custom-idle) causes problems with some parsers
# Because the final version name becomes: 5.16.5-051605+custom-idle-generic, so just keep it combined
...
#
# To uninstall a kernel: $ sudo apt purge *5.16.5-051605+customidle-generic*
